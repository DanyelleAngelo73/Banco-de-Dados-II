  ...
  <head>
    ...
    <title>Despesas no Cartão</title>
    <script>
    ...
    <section id="container">
      ...
      <div class="tabbable" id="transacoes"> <!-- Only required for left/right tabs -->
        <ul class="nav nav-tabs">
          <li class="active" id="myTab"><a href="#tab1" data-toggle="tab">Despesas (Cartão)</a></li>
        ...
        <div class="tab-content">
          <div class="tab-pane active cadastrado" id="tab1">
            ...
            <?php
            $buscaDespesa = $query->sql_query("SELECT * FROM visualizaDespesasCartao where Id_User = {$_SESSION['Id']}");
            if(mysqli_num_rows($buscaDespesa)>0){
              ...
              while($linha=$buscaDespesa->fetch_assoc()){
                ...
                echo  "Cartão: ".$linha['NomeCartao']."<br>";
                ...
                echo "<div id='myModal".$linha['IdTransacao']."' class='modal hide fade' tabindex='-1' role='dialog' aria-labelledby='myModalLabel' aria-hidden='true'>";
                  ...
                  echo "<form action='insereAlteraTransacao.php?acao=altera&codigo=".$linha['IdTransacao']."&tipo=DC' method='POST' class='form-horizontal'>";
                  echo "<div class='control-group'>";
                    echo "<label class='control-label' for='cartão'>Cartão</label>";
                    echo "<div class='controls'>";
                      echo "<select name='cartao' class='required' placeholder='Cartão' value='".$linha['NomeCartao']."'>";
                      $buscaCartao = $query->sql_query("SELECT Id, Nome FROM Cartao where Id_User = {$_SESSION['Id']}");
                        if(mysqli_num_rows($buscaCartao)>0){
                          while($linha1=$buscaCartao->fetch_assoc()){
              ...
              echo  "<h3>Não existem despesas no cartão cadastradas cadastradas!</h3>";
            }
            ?>
          </div>

          <div class="newTransacao tab-pane" id="tab2">
            <h2> Inserir Despesa no Cartão</h2><br>
            <div id="formTransacao">
              <form action="insereAlteraTransacao.php?tipo=DC&acao=insere" method="POST" class="form-horizontal">
                <div class="control-group">
                  <label class="control-label" for="cartão">Cartão</label>
                  <div class="controls">
                    <select name="cartao" class="required" placeholder="Cartão">
                      <?php
                      $buscaCartao = $query->sql_query("SELECT Id, Nome FROM Cartao where Id_User = {$_SESSION['Id']}");
                      if(mysqli_num_rows($buscaCartao)>0){
